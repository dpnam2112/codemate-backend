...
from agents.reading_material.agent import AIGenReadingMaterialSchema
from core.response import Ok
from machine.api.deps import get_current_student
...
Giảm mỡ + Tăng cơ: vừa giảm mỡ, vừa tạo cơ cho tỉ lệ eo sâu, mông căng tròn.
Chuyên sâu mông căng tròn: tạo dáng mông quyến rũ và săn chắc.
        where_=[RecommendQuizzes.id == quizId],
    )
    if not quiz:
        raise NotFoundException(message="Quiz not found for the given ID.")
...
@router.post("/{module_id}/reading-material", response_model=Ok[AIGenReadingMaterialSchema])
async def get_reading_material(
    module_id: UUID,
    current_student: Student = Depends(get_current_student),
    modules_controller: ModulesController = Depends(InternalProvider().get_modules_controller)
):
    #TODO: Endpoint for learner to submit questions generated by AI concerning the reading material
    reading_material = modules_controller.get_reading_material(current_student, module_id)
    return Ok(data=reading_material)

@router.get("/{module_id}/reading-material:submitQuestions")
async def submit_reading_material_and_quiz(
    module_id: UUID,
    current_student: Student = Depends(get_current_student),
    modules_controller: ModulesController = Depends(InternalProvider().get_modules_controller),
):
    """
    Generate reading material and questions to evaluate the student's understanding via LLM. 
    """

@router.post("/{module_id}/reading-material:submitQuestions")
async def get_quiz(
    module_id: UUID,
    current_student: Student = Depends(get_current_student),
    modules_controller: ModulesController = Depends(InternalProvider().get_modules_controller),
