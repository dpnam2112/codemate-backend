...
from sqlalchemy import Column, String, ForeignKey, Enum as DBEnum, Integer, DateTime, Boolean, Float
...
class Exercises(Base):
    ...
    course_id = Column(UUID, ForeignKey("courses.id"), nullable=True) # Foreign key linking the exercise to a specific course in the "courses" table
    ...
    type = Column(DBEnum(ExerciseType), nullable=False) # The type of exercise, represented as an enumerated value (e.g., Quiz, Code)

    # Source of the exercise (manual or generated by AI)
    source: Mapped[ExerciseSource] = mapped_column(String, nullable=False, default=ExerciseSource.MANUAL)

    # If the exercise is generated by AI -> this exercise is linked to a module.
    module_id: Mapped[Optional[UUID]] = mapped_column(
        UUID(as_uuid=True),
        ForeignKey("modules.id", ondelete="CASCADE"),
        nullable=True,
        comment="Module ID for the exercise",
    )
    ...
    grading_method = Column(DBEnum(GradingMethodType), nullable=False, default=GradingMethodType.highest)  # How the final grade is calculated
