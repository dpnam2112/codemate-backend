...
from typing import Any, Dict, Generic, Optional, ParamSpec, Sequence, Tuple, Type, TypeVar
...
class BaseRepository(Generic[ModelType]):
    ...
    async def get_many_with_total(
        ...
        """
        Returns a tuple of records and the total count based on pagination params.

        :param skip: The number of records to skip.
        :param limit: The number of records to return.
        :param fields: Specific fields to include in the result.
        :param join_: The joins to make.
        :param order_: The order of the results.
        :param where_: Filter conditions for the query.
        :return: A tuple containing a list of records and the total count.
        """

        # Prepare the main query for fetching records
        base_query = await self._query(fields=fields, join_=join_, order_=order_, where_=where_)

        # Execute the queries
        records = await self._all(base_query.limit(limit).offset(skip))
        total_count = await self._count(base_query)
