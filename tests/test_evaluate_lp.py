...
@pytest.mark.parametrize("model_name", ["gpt-4o-mini", "gemini/gemini-2.0-flash"])
@pytest.mark.asyncio
async def test_evaluate_all_metrics_batch(model_name: str):
    input_file = "learning_paths_evaluation.json"
    output_file = f"results_learning_path_eval_{model_name.replace('/', '_')}.csv"
